...
class DecideKickEnv(gym.Env):
  ...
  def __init__(self):
    ...
    # })
    # self.observation_space = spaces.Dict({
    #   'aliesArray':np.array([[0,0,0]]*10,dtype=float),
    #   'opponentsArray':np.array([[0,0,0]]*11,dtype=float),
    #   'ballPos':np.array([0,0,0]*10,dtype=float),
    #   'mePos':np.array([0,0,0],dtype=float),
    #   'target':np.array([0,0,0],dtype=float),
    #   'playMode':np.(low=1,high=12,dtype=int)
    # })
    ...
    #   'playMode':np.(low=1,high=12,dtype=int)
    # })
    self.observation_space = spaces.Box(np.array([0, 0]), np.array([60, 300]))

    #PlayModes Generic 1-Att/2-Def, 3 4 KickOff(O/T), 5 6 CornKickIn, 7 8 FKick, 9 10 GFKick, 11 12 Pen
  ...
  def step(self, action, loop=-1): 
    ...
    #except:
     # print("Couldn't get agent message")
